...
def open_main():
    ...
    def display_login():
        ...
        left_frame = tk.Frame(main_content, width=430, height=400, bg="#cbe6d3")
        left_frame.pack(side="left", fill="both", expand=True)
        right_frame = tk.Frame(main_content, width=430, height=500, bg="#cbe6d3")
        right_frame.pack(side="right", fill="both", expand=True)
        image_path = "download.jpeg"  # Ensure this path is correct
        try:
            image = Image.open(image_path).resize((430,450))  # Resize to fit half window
    ...
    def filter_table(product_id):
        ...
        display_table(main_content.winfo_children()[-1], filtered_data)
    ...
    def get_user_input(row):
        ...
        if user_value is not None:
            ...
            elif current_action == "View Today's Sales":
